...
SWEP.PrintName						= "Triple Gold Westar"
SWEP.Type							= "big PP"
SWEP.DrawAmmo						= true
SWEP.data 							= {}
...
SWEP.Attachments = {
	...
	[2] = { 
	header = "Barrels",
	atts = { "extraammo", "suppressor"},
	},
	[3] = { 
	header = "Ammo",
	atts = { "smallpounder", "mediumpounder", "bigppounder", "thermo"},
	},
	[4] = { 
	header = "underBarrels",
	atts = { "squirt", "leak", "gush"},
	},
}--22
local MaxTimer = 55
...
function SWEP:ShootBullet(...)
	if self:IsAttached("smallpounder") or self:IsAttached("mediumpounder") or self:IsAttached("bigppounder") or self:IsAttached("thermo") and IsFirstTimePredicted() then
		if self:IsAttached("smallpounder") then ew = "gb5_light_b_100lbP" end
		if self:IsAttached("mediumpounder") then ew = "gb5_medium_b_500lbP"  end
		if self:IsAttached("bigppounder") then ew = "gb5_medium_b_mk82P"  end
		if self:IsAttached("thermo") then ew = "gb5_medium_m_thermobaricP"  end
		if self:IsAttached("squirt") then lnc = 500  end
		if self:IsAttached("leak") then lnc = 2500  end
		if self:IsAttached("gush") then lnc = 5000  end
			if SERVER then
